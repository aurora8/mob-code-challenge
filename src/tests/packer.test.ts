...
describe('Packer Test suite', () => {
    it('should through ApiError', async () => {
        ...
        try {
            await Packer.readInputFile(join(__dirname, '../../resources/example_input4'));
        } catch (e: any) {
    ...
    it('should test example_input', async () => {
        ...
        const out = await Packer.pack(join(__dirname, '../../resources/example_input'));
    ...
    it('should test example_input and output example_output', async () => {
        ...
        const expected_output = await Packer.readInputFile(resolve(join(__dirname, '..', '..', 'resources/example_output')));
        const out = await Packer.pack(join(__dirname, '../../resources/example_input'));
